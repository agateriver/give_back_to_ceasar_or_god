...
mod tests {
    ...
    fn test_first_quoted_substring() {
        ...
        // 测试带空格的路径
        assert_eq!(
            first_quoted_substring(r#"some text "C:\Program Files (x86)\WPS Office\ksolaunch.exe" more text"#),
            None
        );
...
mod windows_tests {
    ...
    fn test_kso_registry_structure() {
        ...
        if kso_user.is_ok() || kso_system.is_ok() {
            ...
            println!("WPS Office registry structure not found - this is normal if WPS is not installed");
        }
    }
}
